...
def parse_args():
    ...
    p.add_argument("--min-meta", type=float, default=None, help="Exclude quadkeys with meta_baseline < this (e.g. 30)")
...
def main():
    ...
    # -------------------------------------------------------------------------
    # 6b. Optional: Filter by minimum Meta baseline
    # -------------------------------------------------------------------------
    if args.min_meta is not None:
        before = len(meta)
        meta = meta[meta["meta_baseline"] >= args.min_meta].copy()
        print(f"\n--- Filter: meta_baseline >= {args.min_meta} ---")
